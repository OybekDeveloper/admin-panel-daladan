...
import { FaqDeleteModal } from '../../reducer/events'
import { close, line, trashIcon } from '../category/category-img'
import { ApiServices } from '../../services/api.get'
import { toast } from 'react-toastify'
const DeleteModal = () => {

    const { faqDel, deleteFaqId } = useSelector(state => state.events)
    const dispatch = useDispatch()
    const handleClose = () => {
        dispatch(FaqDeleteModal())
    }
    const handleDelete = async () => {
        try {
            const token = localStorage.getItem('token')
            await ApiServices.delData(`faq/${deleteFaqId}`, token);
            dispatch(FaqDeleteModal())
    ...
    return (
        <div>
            <Transition show={faqDel} as={Fragment}>
