...
const Admin = () => {
    ...
    const [people, setPeople] = useState([{ name: "Admin", check: false, role: "ROLE_ADMIN" }, { name: "Moderator", check: false, role: "ROLE_MODERATOR" }])
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await ApiServices.postData('admin/create', formData);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await ApiServices.getData('admin/all')
                setAllAdmin(data)
                console.log(data)
            } catch (err) {
                console.log(err);
    ...
    return (
        <div className="admin px-[24px] py-[32px]">
                            ...
                            <Dialog.Panel className=" rounded-[12px] bg-[#fff] shadow-modal">
                                <div className="p-[24px] flex flex-col items-start">
                                    <div className="w-full flex justify-between items-center">
                                        ...
                                        <img onClick={() => setIsOpen(false)} className="w-[24px] h-[24px] cursor-pointer" src={xicon} alt="icon" />
                                    </div>
                                    {errorMessage && <h1 className='w-full bg-red-200 rounded-[12px] p-[10px] mt-[10px]'>{errorMessage.phone}</h1>}
                                    <form className="w-[360px] flex flex-col gap-[20px] pt-[10px]">
                                        ...
                                        <div className="flex flex-col items-start gap-[6px]">
                                            ...
                                            <button type="button" className="relative w-full flex justify-center items-center   px-[14px] py-[10px] border-[1px] border-solid border-[#D0D5DD] rounded-[8px] focus:outline-[1px] focus:outline-solid outline-[#84caff] focus:shadow-custom">
                                                <div onClick={() => setIsActive(!isActive)} className="flex justify-between items-start w-full">
                                                ...
                                                <motion.div
                                                    ...
                                                    {people.map(item => (
                                                        <div onClick={() => handleActive(item.name)} key={crypto.randomUUID()} className="py-[10px] px-[8px] rounded-[6px] hover:bg-[#F9FAFB] flex justify-between items-center">
                                                            <h1>{item.name}</h1>
                                                            <div>
                                                                {item.check && <img src={check1} alt="sdafasf" />}
                                                            </div>
                                                        </div>
                                                    ))}
                                                </motion.div>
                                            </button>
                                            <button className="w-full mt-[32px] bg-[#1570EF] text-[#fff] text-[16px] font-[600] rounded-[8px] px-[16px] py-[10px] border-[1px] border-solid border-[#1570EF]" onClick={handleSubmit}>
                                                Qo'shish
                                            </button>
                                        </div>
                                    </form>
